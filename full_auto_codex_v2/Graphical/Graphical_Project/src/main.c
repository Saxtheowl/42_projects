...
{
	printf("Usage: %s [scene.rt] [--out output.ppm] [--size WxH] [--samples N] [--threads N] [--gamma G] [--maxdepth D] [--depth depth.ppm] [--normal normal.ppm]\n", prog);
...
int	main(int argc, char **argv)
{
	...
	if (gamma <= 0.0)
		gamma = 2.2;
	if (render_ppm(&scene, out, width, height, samples, threads, gamma, max_depth, depth_out, normal_out))
	{
