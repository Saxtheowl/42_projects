...
output=$(ROOT_OVERRIDE="${stub_root}" "${SCRIPT}" --silent --skip-doctor 2>&1)
if echo "${output}" | rg -q "Skipping doctor|Checks completed"; then
  echo "Expected no skip or completion messages in silent mode" >&2
  exit 1
fi

echo "[ok] run_checks silent skip doctor no message tests passed"
