...
if ! printf '%s' "${output}" | python3 -c 'import json,sys; data=json.loads(sys.stdin.read().strip()); assert data.get("status") == "error"; assert data.get("rabbitmq") == "error"'; then
  echo "Doctor JSON output unexpected for invalid rabbitmq." >&2
  exit 1
fi

echo "[ok] doctor json invalid rabbitmq tests passed"
