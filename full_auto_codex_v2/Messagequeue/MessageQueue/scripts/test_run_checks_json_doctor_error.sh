...
if [ "${status}" -eq 0 ]; then
  echo "Expected non-zero exit on doctor error" >&2
  exit 1
fi
if ! echo "${output}" | rg -q '"doctor": "error"'; then
  echo "Expected doctor error in JSON" >&2
  exit 1
fi

echo "[ok] run_checks json doctor error tests passed"
