...
if ! printf '%s' "${output}" | python3 -c 'import json,sys; data=json.loads(sys.stdin.read().strip()); assert data.get("status") == "ok"; assert isinstance(data.get("queues"), list); assert isinstance(data.get("exchanges"), list); assert isinstance(data.get("bindings"), list)'; then
  echo "status_report JSON invalid payload handling failed." >&2
  exit 1
fi

echo "[ok] status_report json invalid payload tests passed"
