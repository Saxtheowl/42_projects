...
if ! printf '%s' "${output}" | python3 -c 'import json,sys; data=json.loads(sys.stdin.read().strip()); assert data.get("status") == "error"; assert data.get("doctor") == "error"; assert data.get("routing_matrix") == "ok"'; then
  echo "run_checks JSON error output unexpected." >&2
  exit 1
fi

echo "[ok] run_checks json error tests passed"
