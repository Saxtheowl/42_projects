...
if ! printf '%s' "${output}" | python3 -c 'import json,sys; data=json.loads(sys.stdin.read().strip()); assert data.get("status") in ("ok","error"); assert "prereqs" in data and "rabbitmq" in data and "topology" in data'; then
  echo "Doctor JSON output missing fields." >&2
  exit 1
fi

echo "[ok] doctor json tests passed"
