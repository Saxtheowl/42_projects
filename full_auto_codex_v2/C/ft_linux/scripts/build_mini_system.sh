...
SRC="$ROOT/sources"
JOBS="${JOBS:-$(nproc)}"
MANIFEST="$ROOT/configs/mini_system_manifest.tsv"
DRY_RUN=0
RESUME=0
STATE_FILE="$ROOT/work/mini_system.state"
SHOW_STATUS=0
RESET_STATE=0
RANGE_FROM=""
RANGE_UNTIL=""
...
usage() {
	cat <<EOF
Usage: $0 [--dry-run] [--check] [--check-allow-fail] [--resume] [--status] [--reset-state] [--from <pkg>] [--until <pkg>] [--manifest <file>] [--state <file>] {list|all|<package>}
EOF
	exit 1
}
