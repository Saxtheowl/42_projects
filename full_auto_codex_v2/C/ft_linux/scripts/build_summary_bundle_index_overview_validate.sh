...
JSON_FILE="$REPORT_DIR/build_summary_bundle_index_overview.json"
OUT_TXT="$REPORT_DIR/build_summary_bundle_index_overview_validate.txt"

usage() {
	cat <<EOF
Usage: $0 [--json <file>] [--out <file>]

Valide les champs cles du JSON build_summary_bundle_index_overview.
EOF
	exit 1
}
...
{
	echo "build_summary_bundle_index_overview_validate generated: $(date '+%Y-%m-%d %H:%M:%S')"
	echo "json: $JSON_FILE"
	echo ""
} >"$OUT_TXT"
...
missing=0
for field in generated trend delta result; do
...
echo "[OK] Build summary bundle index overview validate generated: $OUT_TXT"
