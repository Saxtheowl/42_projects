...
JSON_FILE="$REPORT_DIR/build_summary_alerts_stats_history_rollup.json"
OUT_TXT="$REPORT_DIR/build_summary_alerts_stats_history_rollup_validate.txt"

usage() {
	cat <<USAGE
Usage: $0 [--report <file>] [--json <file>] [--out <file>]

Valide le rollup historique stats alertes.
USAGE
	exit 1
}
...
{
	echo "build_summary_alerts_stats_history_rollup_validate generated: $(date '+%Y-%m-%d %H:%M:%S')"
...
echo "[OK] Build summary alerts stats history rollup validate generated: $OUT_TXT"
