...
OUT_TXT="$REPORT_DIR/build_summary_alerts_validate.txt"

usage() {
	cat <<EOF
Usage: $0 [--json <file>] [--out <file>]

Valide les champs cles du JSON build_summary_alerts.
EOF
	exit 1
}
...
{
	echo "build_summary_alerts_validate generated: $(date '+%Y-%m-%d %H:%M:%S')"
	echo "json: $JSON_FILE"
	echo ""
} >"$OUT_TXT"
...
missing=0
for field in generated source alerts items bundle_index_score result; do
...
echo "[OK] Build summary alerts validate generated: $OUT_TXT"
