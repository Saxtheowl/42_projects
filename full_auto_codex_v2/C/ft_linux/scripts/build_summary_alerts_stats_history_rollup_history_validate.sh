...
OUT_TXT="$REPORT_DIR/build_summary_alerts_stats_history_rollup_history_validate.txt"

usage() {
	cat <<EOF
Usage: $0 [--csv <file>] [--out <file>]

Valide le CSV build_summary_alerts_stats_history_rollup_history.
EOF
	exit 1
}
...
{
	echo "build_summary_alerts_stats_history_rollup_history_validate generated: $(date '+%Y-%m-%d %H:%M:%S')"
	echo "csv: $CSV_FILE"
	echo ""
} >"$OUT_TXT"
...
echo "[OK] Build summary alerts stats history rollup history validate generated: $OUT_TXT"
