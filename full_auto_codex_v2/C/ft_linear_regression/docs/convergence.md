...
11. `scripts/refresh_validation_artifacts.py` enchaîne `preview_validation.py --archive`, `prune_validation_archives.py`, `verify_archive_summary.py` et `export_validation_summary_csv.py` afin de rafraîchir/archiver/pruner/vérifier/exporter en une seule commande avant la session ft_helpme.
12. `scripts/export_validation_summary_csv.py` peut exporter `docs/validation_summary.json` vers `docs/validation_summary.csv` (timestamp, fold count, best/worst/average), utile pour les outils de reporting ou l’injection dans un tableau de suivi; `scripts/export_validation_summary_yaml.py` produit la même table en YAML pour les notes rapides, et `scripts/refresh_validation_artifacts.py` lance désormais ces exportations à la fin du pipeline.
