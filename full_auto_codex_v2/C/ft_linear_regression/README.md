...
- Un nouveau script `scripts/validation_summary.py` peut lire `data/validation_report.txt` et fournir un résumé rapide (nombre de folds, meilleur/pire RMSE, moyenne) pour faciliter les comptes rendus de la review.
  ...
  - Le dossier `docs/archive/` contient un `README.md` décrivant le fonctionnement des archives HTML (`scripts/preview_validation.py --archive` + `scripts/archive_validation_html.py`) pour que la revue sache quelle version figée consulter; `scripts/list_validation_archives.py` liste les snapshots disponibles, `scripts/prune_validation_archives.py` supprime les plus vieilles, `scripts/verify_archive_summary.py` compare la dernière archive avec `docs/validation_summary.json` pour garantir l’intégrité des statistiques partagées, `scripts/verify_all_validation_archives.py` vérifie tout l’historique, `scripts/diff_validation_archives.py` permet de comparer deux archives HTML pour repérer les différences, `scripts/refresh_validation_artifacts.py` chaîne tous ces outils pour un rafraîchissement complet (avec export CSV/YAML final), `scripts/export_validation_summary_csv.py` fournit `docs/validation_summary.csv` pour alimenter facilement les outils de la revue, `scripts/export_validation_summary_yaml.py` produit `docs/validation_summary.yaml` pour une inclusion légère, et `scripts/index_validation_archives.py` génère `docs/archive/index.md` détaillant RMSE/best/worst par snapshot.
