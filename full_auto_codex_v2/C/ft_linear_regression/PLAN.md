...
- [x] Introduire early stopping (patience, min delta) pour interrompre les runs quand le RMSE ne s’améliore plus, ce qui est utile pour la revue ft_helpme.
...
- [x] Ajouter `scripts/validation.py` : splits aléatoires, RMSE moyen, scheduler/validation config, utile pour la revue/coverage (supporte early stopping + best epoch/train RMSE).
...
- [x] Documenter `scripts/reports/rmse_plot.py` et l’intégrer dans la roadmap post-review.
*** End Patch
