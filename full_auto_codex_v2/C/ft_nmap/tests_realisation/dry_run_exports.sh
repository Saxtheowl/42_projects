...
grep -q "version,0.2.5" "$csv" || { echo "version missing in CSV header"; exit 1; }
grep -q "dry_run: true" "$yaml" || { echo "dry_run missing in YAML"; exit 1; }
grep -q "version: \"0.2.5\"" "$yaml" || { echo "version missing in YAML"; exit 1; }
grep -q 'dry_run="true"' "$xml" || { echo "dry_run missing in XML"; exit 1; }
grep -q 'version="0.2.5"' "$xml" || { echo "version missing in XML"; exit 1; }
grep -q "dry_run: true" "$md" || { echo "dry_run missing in Markdown report"; exit 1; }
grep -q "\\*\\*Version\\*\\*: 0.2.5" "$md" || { echo "version missing in Markdown report"; exit 1; }
