...
{
	printf("Usage: %s -t target [-p ports|-P file|-k top] [-x ports|-X file] [-T timeout_ms] [-c inflight] [-R retries] [-b backoff_pct] [-w delay_ms] [-M deadline_ms] [-f stop_after_open| -F] [-q] [-O] [-S] [-l] [-r] [-e seed] [-g progress_ms] [-I ip_override] [-4|-6] [-n] [-o file.json] [-J summary.json] [-C file.csv] [-N file.ndjson] [-Y file.yaml] [-Z file.xml] [-H file.html] [-m file.md] [-L open_ports.txt] [-E export_filter] [-Q] [-V]\n",
		prog);
	printf("  -t target     Target hostname or IPv4 address (required)\n");
	printf("  -p ports      Comma-separated list or ranges (e.g. 22,80-90,443)\n");
printf("  -P file       File containing ports/ranges (whitespace/comma separated, last -p/-P wins)\n");
	printf("  -k top        Scan the TOP <n> common TCP ports (built-in list)\n");
printf("  -x ports      Exclude ports/ranges from the scan\n");
printf("  -X file       File with ports/ranges to exclude (last -x/-X wins)\n");
