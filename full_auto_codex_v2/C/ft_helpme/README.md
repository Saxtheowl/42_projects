...
- Pour comparer deux snapshots HTML archivés (chronologies, tests), utilisez `python3 C/ft_linear_regression/scripts/diff_validation_archives.py validation_summary_<old>.html validation_summary_<new>.html`.
- Pour comparer deux snapshots HTML archivés (chronologies, tests), utilisez `python3 C/ft_linear_regression/scripts/diff_validation_archives.py validation_summary_<old>.html validation_summary_<new>.html`, puis envoyez celui qui montre le comportement souhaité à la revue ft_helpme.
- Pour vous assurer qu’aucun archive n’est obsolète, lancez `python3 C/ft_linear_regression/scripts/verify_all_validation_archives.py` : il compare chaque HTML de `docs/archive/` aux métriques `docs/validation_summary.json`.
