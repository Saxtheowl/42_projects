...
./scripts/logs_metrics_pipeline.sh "${pipeline_args[@]}"

target_csv="${REPORTS_DIR}/log_metrics_snapshot.${SUFFIX}.csv"
if [ -n "$COMPARE_BASE" ]; then
  echo "[ci] Comparing ${COMPARE_BASE} -> ${target_csv}"
  python3 ./scripts/logs_metrics_compare.py --base "$COMPARE_BASE" --target "$target_csv" --output "${REPORTS_DIR}/log_metrics_compare.md"
  python3 ./scripts/logs_metrics_compare_html.py --base "$COMPARE_BASE" --target "$target_csv" --output "${REPORTS_DIR}/log_metrics_compare.html"
fi

echo "[ci] Exporting JSONL"
python3 ./scripts/logs_metrics_snapshot_to_jsonl.py --input "$target_csv" --output "${target_csv%.csv}.jsonl"
