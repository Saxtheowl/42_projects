...
{
	std::cout << "ClapTrap name ctor" << std::endl;
}
...
{
	std::cout << "ClapTrap copy assign" << std::endl;
	if (this != &other)
	{
		_name = other._name;
...
void ClapTrap::attack(const std::string &target)
{
	...
	std::cout << "ClapTrap " << _name << " attacks " << target << " for " << _attackDamage << " dmg" << std::endl;
	--_energyPoints;
}
...
void ClapTrap::beRepaired(unsigned int amount)
{
	...
	_hitPoints += static_cast<int>(amount);
	--_energyPoints;
	std::cout << "ClapTrap " << _name << " repairs " << amount << " HP (HP=" << _hitPoints << ")" << std::endl;
}
